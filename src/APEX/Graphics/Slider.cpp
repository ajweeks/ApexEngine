...
namespace apex
{
	const float Slider::GRIP_MOVE_MS_PER_PIXEL = 8.5f;
	const sf::Int32 Slider::GRIP_COLOR_TRANSITION_MS = 90;
	const Transition::EaseType Slider::GRIP_MOVE_EASE_TYPE = Transition::EaseType::CIRCULAR_OUT;
	const float Slider::GRIP_REL_HEIGHT = 1.4f; // The grip is 1.4 times taller than the bg rect
	const float Slider::GRIP_REL_WIDTH = 1.0f / 15.0f; // The grip is 15 times less wide than the bg rect

	Slider::Slider(float width, float height, float left, float top, float initalValue)
	{
	...
	void Slider::Tick(sf::Time elapsed)
	{
	...
	void Slider::Draw(sf::RenderTarget& target, sf::RenderStates states)
	{
		target.draw(m_BackgroundRectangle, states);
		target.draw(m_GripRectangle, states);
	}

	void Slider::CreateGripTransformableTransition(sf::Vector2f endPos, bool instant)
	{
		CreateGripTransformableTransition(endPos.x, endPos.y, instant);
	}

	void Slider::CreateGripTransformableTransition(float endPosX, float endPosY, bool instant)
	{
		const sf::Vector2f gripPos = m_GripRectangle.getPosition();
	...
	bool Slider::BeingDragged() const
	{
		return m_BeingDragged;
	}

	bool Slider::OnButtonPress(sf::Event::MouseButtonEvent buttonEvent)
	{
